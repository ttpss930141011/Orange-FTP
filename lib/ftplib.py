...
class FTP:
    ...
    def getwelcome(self):
        ...
        if self.debugging:
            welcomeMsg = f'*welcome* {self.sanitize(self.welcome)}'
    ...
    def putline(self, line):
        ...
        line = line + CRLF
        if self.debugging > 1:
            self.sendMessage(f'*put* {self.sanitize(line)}')
    ...
    def putcmd(self, line):
        if self.debugging: 
            self.sendMessage(f'*cmd* {self.sanitize(line)}')
    ...
    def getline(self):
        ...
        if self.debugging > 1:
            self.sendMessage(f'*get* {self.sanitize(line)}')
    ...
    def getresp(self):
        ...
        if self.debugging:
            self.sendMessage(f'*resp* {self.sanitize(resp)}')
    ...
    def abort(self):
        ...
        line = b'ABOR' + B_CRLF
        if self.debugging > 1:
            self.sendMessage(f'*put urgent* {self.sanitize(line)}')
        ...
        with self.transfercmd(cmd) as conn, \
            ...
            while 1:
                ...
                if len(line) > self.maxline:
                    raise Error("got more than %d bytes" % self.maxline)
                if self.debugging > 2:
                    self.sendMessage(f'*retr* {repr(line)}')
    ...
    def sendMessage(self,text):
        pub.sendMessage("sendMsg", msg=text)
